...
def evaluate_translation_with_qwen(source_text: str, 
    ...
    """Qwen-3 모델을 사용하여 번역을 평가하고 MQM 점수를 추출합니다."""
    
    # 프롬프트 생성
    if prompt_type == 'gemba_mqm':
        prompt = create_gemba_mqm_prompt(source_text, translation)
    elif prompt_type == 'ea_prompt':
        prompt = create_ea_prompt(source_text, translation)
    else:
